...
public class WaitListController {
    ...
    @GetMapping
    public ResponseEntity<?> getWaitListBySemesterAndYear(@RequestHeader("id") String studentId, @RequestParam int semester, @RequestParam int year) {
    ...
    public ResponseEntity<?> registerWaitList(@RequestHeader("id") String studentId, @RequestBody RegisterWaitListRequest request) {
        List<Course> courses = courseClient.getCoursesByIds(List.of(request.courseId()));
        if (courses.isEmpty()) {
